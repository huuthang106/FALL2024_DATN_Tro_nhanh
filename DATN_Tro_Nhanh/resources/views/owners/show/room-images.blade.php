...
@section('titleOwners', 'Ảnh của phòng trọ')
                    ...
                    @foreach ($images as $image)
                        <tr class="shadow-hover-xs-2 bg-hover-white">
                            <td class="align-middle pt-6 pb-4 px-6">
                                <div class="media">
                                    <div class="w-120px mr-4 position-relative">
                                        <img src="{{ asset('assets/images/' . $image->filename) }}" alt="Image" class="img-fluid">
                                    </div>
                            ...
                            <td class="align-middle">
                                <form action="{{ route('owners.delete-room-image', ['id' => $image->id]) }}" method="POST" class="d-inline-block delete-image-form">
                                    @csrf
                                    @method('DELETE')
        ...
        @if ($images->lastPage() > 1)
            <ul class="pagination rounded-active justify-content-center">
                ...
                @if ($images->currentPage() < $images->lastPage() - 1)
                    <li class="page-item">
                        <a class="page-link" href="{{ $images->url($images->lastPage()) }}">{{ $images->lastPage() }}</a>
                    </li>
                @endif
